...
// okay so my snake "Billy" is a linked list 

// simple Node definition nothing complex, value and next pointer
...
// I decided to use both head and tail pointers helps to move the snake in O(1) time
class LinkedList {
  constructor(value) {
...
//2 speed options only
const Speeds = {
  Hard: 90,
  Easy: 150,
};

//array to store the valid directions
const Direction = {
  UP: 'UP',
  RIGHT: 'RIGHT',
  DOWN: 'DOWN',
...
// where by default my snake would appear at the start of the game situation
...
const moveSnake = () => {
  ...
  else if (snakeArray.length > 1 && (snakeArray[snakeArray.length - 2] === nextHeadCell || snakeArray[snakeArray.length - 3] === nextHeadCell || snakeArray[0] === nextHeadCell)){
    setDirection(getOppositeDirection(direction));
    nextHeadCoords = getCoordsInDirection(currentHeadCoords, getOppositeDirection(direction));
...
//handles new food generation + score 
...
return (
  <>
    ...
    <div>
      <h2>Instructions</h2> 
      <div className="infotext">1. Move your snake around using the arrow keys</div>
      <div className="infotext">2. Feed your snake.</div>
      <div className="infotext">3. Purple food is poisoned and will invert your snake.</div>
      <div className="infotext">4. Bumping on wall or its body will kill the snake</div>
        <div className="rectangle back" onClick={() => setInfo(!info)}>Back </div>
    </div>
    </>
    :
    <>
    ...
    <div className="start">
      <div className="speeds">
        <div className={`rectangle ${speed === Speeds.Hard ? "active" : ""}`} onClick={() => setSpeed(Speeds.Hard)}>Hard Mode</div>
        <div className={`rectangle ${speed === Speeds.Easy ? "active" : ""}`} onClick={() => setSpeed(Speeds.Easy)}>Easy Mode</div>
      </div>
      ...
      <div className="info-line">
        <div className="info" onClick={() => setInfo(!info)}>Instructions</div>
      </div> 
    </div>
    </>
    :
